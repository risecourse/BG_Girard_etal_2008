...
daWeight = 1 # scale the weight of the synapses onto the striatum neurons
			 # that receive dopaminergic input (represents # dopamine
			 # receptors or increased levels of dopamine)
...
class CBGTC:
  ...
  def __init__(self,model='CBG',NbChannels=6,opt_params=[]):
    ...
    self.BG = basalganglia.BasalGanglia(model,NbChannels,opt_params[:18],simname)
  ...
  def evaluate2ChannelsCompetition(self,dt,verbose='stfu'):
...
#  myCBGTC.evaluate2ChannelsCompetition(dt) # can be pretty long
  ...
  #=========================================
  CBGparams = [
    0.9*daWeight,  # S -> D1/D2 synaptic weight
    0.1*daWeight,  # FC -> D1/D2 synaptic weight
    0.09, # S -> FS
    0.01, # FC -> FS
    0.7,  # STN -> GPE/GPi
    0.45, # GPe -> STN
    ...
    0.4,  # D1 -> GPi
    0.4,  # D2 -> GPe
    0.5*daWeight,  # FS -> D1/D2
    0.58, # FC -> STN
    0.1,  # -I_D1/D2  up/down state (so doesn't have scaling weight applied to it)
    0.5,  # I_STN
    0.1,  # I_GPe/GPi
    0.18, # BG-> Th
    0.6,  # FC -> Th
  ...
  if pflag:
    ...
    vflag='v'
  customCBG.simpleTest(dt,vflag)
  customCBG.TwoHundredMotelsTest(dt,200,'v')
  customCBG.evaluate2ChannelsCompetition(dt) # can be pretty long
  
  if pflag:
    plotresults.plotnow(simname = simname, NbChannels=6)
